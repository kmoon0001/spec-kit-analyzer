    ...
    try:
        ...
        class ProfessionalNERWindow(QMainWindow):
            ...
            def setup_ui(self):
                ...
                subtitle_label = QLabel("Professional NER Analysis Suite - spaCy-Free Implementation")
                ...
                # Status bar
                self.statusBar().showMessage("Ready - Professional NER system initialized")
...
⚡ Performance: spaCy-free implementation"""
                ...
                for line_result in results['lines']:
                    detailed_text += f"Line {line_result['number']}: {line_result['text']}\n"
                    if line_result['clinicians']:
                        detailed_text += f"   👨‍⚕️ Clinicians: {', '.join(line_result['clinicians'])}\n"
                    else:
                        detailed_text += f"   ℹ️ No clinicians detected\n"
                    detailed_text += "\n"
                
                detailed_text += "\n" + "═" * 50 + "\n"
                detailed_text += "✅ Analysis completed successfully using regex-based extraction\n"
                detailed_text += "🔒 No PHI was transmitted - all processing local\n"
                
                self.detailed_text.setText(detailed_text)
                
                # Update status
                self.statusBar().showMessage(f"Complete - Found {summary['total_clinicians']} clinician(s)")
