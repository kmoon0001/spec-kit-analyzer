    ...
    try:
        ...
        if ext == ".pdf":
            if not pdfplumber:
                return [
                    {
                        "sentence": "Error: pdfplumber not available.",
                        "window": "",
                        "metadata": {"source": "PDF Parser"},
                    }
                ]
            ...
            try:
                ...
                return [
                    {
                        "sentence": "Error: python-docx not available.",
                        "window": "",
                        "metadata": {"source": "DOCX Parser"},
                    }
                ]
            ...
            try:
                ...
                return [
                    {
                        "sentence": f"Error: Failed to read tabular file: {e}",
                        "window": "",
                        "metadata": {"source": "Data Parser"},
                    }
                ]
            ...
            if not Image or not pytesseract:
                return [
                    {
                        "sentence": "Error: OCR dependencies not available.",
                        "window": "",
                        "metadata": {"source": "OCR Parser"},
                    }
                ]
            try:
                ...
                return [
                    {
                        "sentence": f"Error: Unidentified image: {e}",
                        "window": "",
                        "metadata": {"source": "OCR Parser"},
                    }
                ]
            ...
            return [
                {
                    "sentence": f"Error: Unsupported file type: {ext}",
                    "window": "",
                    "metadata": {"source": "File Handler"},
                }
            ]
        ...
        return (
            ...
            if chunks
                ...
                {
                    "sentence": "Info: No text could be extracted from the document.",
                    "window": "",
        ...
        return [
            {
                "sentence": f"Error: An unexpected error occurred: {e}",
                "window": "",
                "metadata": {"source": "System"},
            }
        ]
...
def parse_text_into_sections(text: str) -> Dict[str, str]:
    ...
    pattern = (
        r"^\s*(" + "|".join(re.escape(header) for header in section_headers) + r")\s*:"
    )
    ...
    for i, match in enumerate(matches):
        ...
        canonical_header = next(
            (h for h in section_headers if h.lower() == section_header.lower()),
            section_header,
        )
