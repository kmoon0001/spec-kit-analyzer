...
class ResourceMonitor(QObject):
    ...
    def can_start_job(self, job_type: str = "general") -> tuple[bool, str]:
        ...
        if metrics.ram_percent > self.limits.ram_critical_threshold:
            return (
                False,
                f"Insufficient RAM: {metrics.ram_percent:.1f}% used (critical threshold: {self.limits.ram_critical_threshold}%)",
            )
        ...
        if job_type == "analysis" and metrics.ram_available_mb < self.limits.min_ram_for_analysis:
            return (
                False,
                f"Insufficient RAM for analysis: {metrics.ram_available_mb:.0f}MB available (need {self.limits.min_ram_for_analysis:.0f}MB)",
            )

        if job_type == "model_load" and metrics.ram_available_mb < self.limits.min_ram_for_model_load:
            return (
                False,
                f"Insufficient RAM for model loading: {metrics.ram_available_mb:.0f}MB available (need {self.limits.min_ram_for_model_load:.0f}MB)",
            )
        ...
        if metrics.cpu_percent > self.limits.cpu_critical_threshold:
            return (
                False,
                f"CPU overloaded: {metrics.cpu_percent:.1f}% usage (critical threshold: {self.limits.cpu_critical_threshold}%)",
            )
    ...
    def _update_metrics(self):
        ...
        if metrics.ram_percent >= self.limits.ram_critical_threshold:
            if not self._in_critical_state:
                ...
                self.critical_alert.emit(
                    "RAM", f"Critical RAM usage: {metrics.ram_percent:.1f}% - New jobs will be denied"
                )
        elif metrics.ram_percent >= self.limits.ram_warning_threshold:
            if not self._in_warning_state:
                self._in_warning_state = True
                self.warning_issued.emit("RAM", f"High RAM usage: {metrics.ram_percent:.1f}%")
        else:
            # Resources recovered
        ...
        if metrics.cpu_percent >= self.limits.cpu_critical_threshold:
            if not self._in_critical_state:
                self.critical_alert.emit("CPU", f"Critical CPU usage: {metrics.cpu_percent:.1f}%")
        elif metrics.cpu_percent >= self.limits.cpu_warning_threshold:
            if not self._in_warning_state:
                self.warning_issued.emit("CPU", f"High CPU usage: {metrics.cpu_percent:.1f}%")
