...
class AnalysisWorker(BaseWorker):
    ...
    def _load_document(self) -> Dict[str, Any]:
        ...
        if file_size > max_size:
            raise ValueError(f"Document too large: {file_size / (1024*1024):.1f}MB (max: {max_size / (1024*1024):.0f}MB)")
    ...
    def _classify_document(self, upload_result: Dict) -> Dict:
        ...
        try:
            response = self.api_client.classify_document(
    ...
    def _extract_entities(self, upload_result: Dict) -> Dict:
        ...
        try:
            response = self.api_client.extract_entities(
                document_id=upload_result.get('id'),
                timeout=60
            )
            return response
