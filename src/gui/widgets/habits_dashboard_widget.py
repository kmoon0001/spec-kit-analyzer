...
class HabitProgressBar(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.setFont(font)
        painter.drawText(self.width() - 120, 15, f"{self.percentage:.1f}% - {self.mastery_level}")
...
class HabitDetailsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.principle_label.setStyleSheet("color: #7f8c8d; font-style: italic; margin-bottom: 15px;")
...
class HabitsDashboardWidget(QWidget):
    ...
    def create_right_panel(self) -> QWidget:
        ...
        for habit_id in sorted(self.habits_framework.HABITS.keys(), 
    ...
    def update_progress_display(self):
        ...
        for habit_id in sorted(self.current_metrics["habit_breakdown"].keys(),
                              key=lambda x: int(x.split("_")[1])):
