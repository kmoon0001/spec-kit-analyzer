    ...
    def from_dict(cls, data: dict[str, Any]) -> "BrandingConfiguration":
        ...
        if "logo" in data:
            ...
            config.logo = LogoConfiguration(
                ...
                position=LogoPosition(logo_data["position"]) if logo_data.get("position") else LogoPosition.TOP_RIGHT,
                size=LogoSize(logo_data["size"]) if logo_data.get("size") else LogoSize.MEDIUM,
        ...
        config.footer_text = data.get("footer_text", "Generated by Therapy Compliance Analyzer")
...
class LogoProcessor:
    ...
    def _process_svg_logo(self, path: Path, config: LogoConfiguration) -> dict[str, Any]:
        ...
        if config.opacity < 1.0:
            ...
            svg_content = svg_content.replace("<svg", f'<svg style="opacity: {config.opacity}"')
...
class ReportBrandingService:
    ...
    def get_logo_data(self) -> dict[str, Any] | None:
        ...
        if not self.branding_config.logo.enabled or not self.branding_config.logo.file_path:
            return None
