...
class BrandingConfiguration:
    ...
    logo: LogoConfiguration = field(
        default_factory=LogoConfiguration
    )  # Alias for tests
    ...
    custom_css: str = ""
    footer_text: str = "Generated by Therapy Compliance Analyzer"
    watermark_text: str = ""

    def __post_init__(self):
        # Keep logo and logo_config in sync
        if self.logo != self.logo_config:
            self.logo = self.logo_config
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            "logo": {
                ...
                "file_path": self.logo.file_path,
                "position": self.logo.position.value if self.logo.position else None,
                "size": self.logo.size.value if self.logo.size else None,
                "opacity": self.logo.opacity,
                "margin_top": self.logo.margin_top,
                "margin_right": self.logo.margin_right,
                "margin_bottom": self.logo.margin_bottom,
    ...
    def from_dict(cls, data: dict[str, Any]) -> "BrandingConfiguration":
        ...
        if "logo" in data:
            logo_data = data["logo"]
            config.logo = LogoConfiguration(
                enabled=logo_data.get("enabled", False),
                file_path=logo_data.get("file_path"),
                position=(
                    LogoPosition(logo_data["position"])
                    if logo_data.get("position")
                    else LogoPosition.TOP_RIGHT
                ),
                size=(
                    LogoSize(logo_data["size"])
                    if logo_data.get("size")
                    else LogoSize.MEDIUM
                ),
                opacity=logo_data.get("opacity", 1.0),
                margin_top=logo_data.get("margin_top", 10),
                margin_right=logo_data.get("margin_right", 10),
                margin_bottom=logo_data.get("margin_bottom", 10),
                margin_left=logo_data.get("margin_left", 10),
            )
        ...
        config.report_title_template = data.get(
            "report_title_template", "Compliance Analysis Report"
        )
        ...
        config.footer_text = data.get(
            "footer_text", "Generated by Therapy Compliance Analyzer"
        )
        config.watermark_text = data.get("watermark_text", "")
...
class LogoProcessor:
    ...
    def validate_logo_file(self, file_path: str) -> tuple[bool, str | None]:
        ...
        try:
            ...
            if path.stat().st_size > self.MAX_FILE_SIZE:
                return (
                    False,
                    f"Logo file too large. Maximum size: {self.MAX_FILE_SIZE // (1024 * 1024)}MB",
                )
    ...
    def _process_raster_logo(
        self, path: Path, config: LogoConfiguration
    ) -> dict[str, Any]:
        ...
        with Image.open(path) as img:
            ...
            if (
                config.size == LogoSize.CUSTOM
                and config.custom_width
                and config.custom_height
            ):
    ...
    def _process_svg_logo(
        ...
        if config.opacity < 1.0:
            ...
            svg_content = svg_content.replace(
                "<svg", f'<svg style="opacity: {config.opacity}"'
            )
...
class ReportBrandingService:
    ...
    def __init__(self, config_path: str | Path | None = None):
        self.config_path = (
            Path(config_path) if config_path else Path("config/branding.yaml")
        )
    ...
    def get_branding_context(self) -> dict[str, Any]:
        ...
        context = {
            "branding": {
                ...
                "logo_data": (
                    self.get_logo_data() if self.branding_config.logo.enabled else None
                ),
    ...
    def get_logo_data(self) -> dict[str, Any] | None:
        ...
        if (
            not self.branding_config.logo.enabled
            or not self.branding_config.logo.file_path
        ):
            return None
