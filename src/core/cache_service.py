...
class MemoryAwareLRUCache:
    ...
    def _cleanup_if_needed(self):
        ...
        if system_memory > 80 or actual_cache_size > self.max_memory_mb:
            logger.info("Memory cleanup triggered. System: %.1f%%, Cache: %.1f MB", 
                       system_memory, actual_cache_size)
            ...
            keys_to_remove = embedding_keys[:cleanup_count//2] + other_keys[:cleanup_count//2]
...
def get_cache_stats() -> Dict[str, Union[int, float]]:
    ...
    return {
        ...
        'embedding_entries': len([k for k in cache_service.cache.keys() if k.startswith('embedding_')]),
        'ner_entries': len([k for k in cache_service.cache.keys() if k.startswith('ner_')]),
        'llm_entries': len([k for k in cache_service.cache.keys() if k.startswith('llm_')]),
        'doc_entries': len([k for k in cache_service.cache.keys() if k.startswith('doc_') or k.startswith('chunks_')]),
    }
...
def cleanup_all_caches():
    ...
    logger.info("Cache cleanup completed. Entries: %d -> %d, Memory: %.1f MB -> %.1f MB",
                stats_before['total_entries'], stats_after['total_entries'],
                stats_before['memory_usage_mb'], stats_after['memory_usage_mb'])
