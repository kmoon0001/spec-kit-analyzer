...
class IndividualHabitTracker:
    ...
    async def get_habit_timeline(
        ...
        return {
            ...
            "summary": {
                ...
                "avg_findings_per_day": sum(d["habit_findings"] for d in sorted_timeline) / max(len(sorted_timeline), 1)
            }
        }
    ...
    async def _calculate_personal_insights(
        ...
        if not findings:
            return {"improvement_trend": "No data", "strongest_habits": [], "focus_areas": []}
