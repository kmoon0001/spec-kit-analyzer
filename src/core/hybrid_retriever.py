...
class HybridRetriever:
    ...
    def __init__(self, dense_model_name="pritamdeka/S-PubMedBert-MS-MARCO"):
        ...
        if not self.rules:
            logging.warning(
                "No rules loaded from the database. Retriever will be empty."
            )
            self.corpus = []
            self.bm25 = None
        ...
        logging.info(
            f"Created a corpus with {len(self.corpus)} rules from the database."
        )
    ...
    def retrieve(self, query: str, top_k: int = 5, **kwargs) -> List[Dict]:
        ...
        cosine_scores = (
            cos_sim(query_embedding, self.corpus_embeddings)[0].cpu().numpy()
        )

        # --- Reciprocal Rank Fusion (RRF) for combining results ---
        k = 60
        bm25_ranks = {
            i: rank + 1 for rank, i in enumerate(np.argsort(bm25_scores)[::-1])
        }
        dense_ranks = {
            i: rank + 1 for rank, i in enumerate(np.argsort(cosine_scores)[::-1])
        }
