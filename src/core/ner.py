...
class NERAnalyzer:
    ...
    def extract_clinician_name(self, text: str) -> List[str]:
        ...
        try:
            ...
            for match in re.finditer(title_name_pattern, text, re.IGNORECASE):
                title_and_name = match.group().strip()
            ...
            for keyword_match in re.finditer(clinical_keywords_pattern, text, re.IGNORECASE):
                ...
                for name_match in re.finditer(name_pattern, context):
                    ...
                    if (len(name.split()) >= 2 and 
                        not self._is_likely_patient_name(name, text) and
                        self._is_likely_clinician_name(name, context)):
            ...
            if self.presidio_analyzer:
                ...
                for entity in presidio_entities:
                    if entity.get('entity_group') == 'PERSON':
                        ...
                        if (re.search(clinical_keywords_pattern, context, re.IGNORECASE) and
                            not self._is_likely_patient_name(name, text)):
                            clinician_names.append(name)
    ...
    def _is_likely_patient_name(self, name: str, full_text: str) -> bool:
        ...
        patient_indicators = r'\b(?:patient|pt\.?|client|individual|person|mr\.?|mrs\.?|ms\.?)\b'
    ...
    def _is_likely_clinician_name(self, name: str, context: str) -> bool:
        ...
        clinician_indicators = r'\b(?:dr\.?|doctor|therapist|clinician|provider|signed|signature|by)\b'
    ...
    def _clean_clinician_name(self, name: str) -> str:
        ...
        name = re.sub(r',?\s*(?:pt|ot|slp|dpt|otr|ccc-slp|md|rn|lpn)\.?$', '', name, flags=re.IGNORECASE)
    ...
    def _extract_regex_entities(self, text: str, categorized: Dict[str, List[str]]) -> None:
        """Extract entities using regex patterns for clinical terms."""
        try:
            # Extract medical conditions
            for match in re.finditer(self.clinical_patterns['medical_conditions'], text, re.IGNORECASE):
            ...
            for match in re.finditer(self.clinical_patterns['body_parts'], text, re.IGNORECASE):
            ...
            for match in re.finditer(self.clinical_patterns['therapy_terms'], text, re.IGNORECASE):
            ...
            for match in re.finditer(self.clinical_patterns['measurements'], text, re.IGNORECASE):
