    ...
    def optimize_for_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.cache_service:
                ...
                memory_freed = cache_stats_before.get('memory_usage_mb', 0) - cache_stats_after.get('memory_usage_mb', 0)
                if memory_freed > 0:
    ...
    def _check_performance(self):
        ...
        try:
            ...
            if memory_percent > 90:
                self._emit_warning('critical', f'Critical memory usage: {memory_percent:.1f}%')
            elif memory_percent > 80:
                self._emit_warning('warning', f'High memory usage: {memory_percent:.1f}%')
            
            # Check process memory
            process_memory = memory_stats.get('process_memory_mb', 0)
            if process_memory > 2048:  # 2GB
                self._emit_warning('warning', f'High process memory usage: {process_memory:.1f} MB')
