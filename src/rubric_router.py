...
def create_rubric(rubric: RubricCreate):
    try:
        with get_db_connection() as conn:
            cur = conn.cursor()
            cur.execute("INSERT INTO rubrics (name, content) VALUES (?, ?)", (rubric.name, rubric.content))
            conn.commit()
...
def get_rubrics():
    with get_db_connection() as conn:
        ...
        rows = cur.fetchall()
        return [Rubric(id=row['id'], name=row['name'], content=row['content']) for row in rows]
...
def update_rubric(rubric_id: int, rubric: RubricCreate):
    with get_db_connection() as conn:
        cur = conn.cursor()
        cur.execute("UPDATE rubrics SET name = ?, content = ? WHERE id = ?", (rubric.name, rubric.content, rubric_id))
        conn.commit()
        if cur.rowcount == 0:
            raise HTTPException(status_code=404, detail="Rubric not found")
...
def delete_rubric(rubric_id: int):
    with get_db_connection() as conn:
        ...
        rubric_to_delete = Rubric(id=row['id'], name=row['name'], content=row['content'])
