...
async def get_habit_progression(
    ...
    db: AsyncSession = Depends(get_async_db)) -> schemas.HabitProgressData:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Failed to retrieve habit progression data") from None


@router.get("/summary", response_model=schemas.UserProgressSummary)
async def get_progress_summary(
    current_user: models.User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_async_db)) -> schemas.UserProgressSummary:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve progress summary") from None
...
async def get_user_goals(
    ...
    current_user: models.User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_async_db)) -> list[schemas.HabitGoal]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve goals") from None
...
async def create_goal(
    ...
    current_user: models.User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_async_db)) -> schemas.HabitGoal:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to create goal") from None
...
async def update_goal_progress(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to update goal progress") from None


@router.get("/achievements", response_model=list[schemas.HabitAchievement])
async def get_user_achievements(
    current_user: models.User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_async_db)) -> list[schemas.HabitAchievement]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve achievements") from None
...
async def get_weekly_trends(
    ...
    current_user: models.User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_async_db)) -> list[schemas.WeeklyHabitTrend]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve weekly trends") from None


@router.get("/recommendations", response_model=list[schemas.HabitRecommendation])
async def get_habit_recommendations(
    current_user: models.User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_async_db)) -> list[schemas.HabitRecommendation]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve recommendations") from None
...
async def get_habit_details(
    ...
    current_user: models.User = Depends(get_current_active_user)) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve habit details") from None
