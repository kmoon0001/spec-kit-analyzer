...
async def get_personal_habit_profile(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to generate habit profile: {e!s}") from e
...
async def get_habit_timeline(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to generate habit timeline: {e!s}") from e
...
async def get_personal_goals(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get goals: {e!s}") from e
...
async def create_personal_goal(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid data format: {e!s}") from e
    except (ImportError, ModuleNotFoundError) as e:
        logger.exception("Failed to create goal for user %s", current_user.id)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to create goal: {e!s}") from e
...
async def get_personal_achievements(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get achievements: {e!s}") from e
...
async def get_personal_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get statistics: {e!s}") from e
...
async def create_progress_snapshot(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to create snapshot: {e!s}") from e
