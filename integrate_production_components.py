...
"""
    ...
    if "self.resource_monitor = ResourceMonitor()" not in content:
        ...
        content = re.sub(
            r'(self\._initialize_ui_attributes\(\))',
            r'\1' + init_addition,
            content,
            count=1
        )
...
def update_run_gui_script():
    ...
    with open(run_gui_path, 'r', encoding='utf-8') as f:
        content = f.read()
...
"""
    ...
    if "pycharm_theme.get_application_stylesheet()" not in content:
        ...
        content = re.sub(
            r'(app = QApplication\(sys\.argv\))',
            r'\1' + theme_application,
            content,
            count=1
        )
