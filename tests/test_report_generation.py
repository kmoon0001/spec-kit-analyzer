...
def test_report_generation():
    ...
    try:
        # 1. Initialize the AnalysisService with mocked services
        with patch('src.core.analysis_service.GuidelineService') as mock_guideline_service, \
             patch('src.core.analysis_service.LLMComplianceAnalyzer') as mock_llm_analyzer:

            mock_guideline_service.return_value.search.return_value = []
            mock_llm_analyzer.return_value.analyze_document.return_value = "LLM Analysis"
            ...
            assert "<h2>LLM Analysis</h2>" in report_html
            assert "LLM Analysis" in report_html
