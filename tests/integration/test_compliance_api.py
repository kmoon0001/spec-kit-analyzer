...
class TestComplianceAPI(unittest.TestCase):
    ...
    def test_evaluate_endpoint_success_one_finding(self):
        ...
        self.assertEqual(len(data["findings"]), 1, f"Expected 1 finding, but got {len(data['findings'])}")
        self.assertEqual(data["findings"][0]["rule"]["issue_title"], "Goals may not be measurable/time-bound")
    ...
    def test_evaluate_endpoint_fully_compliant(self):
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data["is_compliant"], f"Expected document to be compliant, but it had findings: {[f['rule']['issue_title'] for f in data['findings']]}")
    ...
    def test_evaluate_endpoint_missing_signature(self):
        ...
        self.assertFalse(data["is_compliant"])
        self.assertEqual(len(data["findings"]), 1, f"Expected 1 finding, but got {len(data['findings'])}")
        self.assertIn("signature/date possibly missing", data["findings"][0]["rule"]["issue_title"].lower())
