...
def mock_backend_services():
    ...
    with patch('src.gui.main_window.AILoaderWorker'), \
         ...
         patch('src.gui.main_window.QMessageBox') as mock_msg_box: # Mock message box to prevent popups
        yield mock_msg_box
...
def test_large_file_upload(main_app_window: MainApplicationWindow, qtbot, tmp_path):
    ...
    with patch('src.gui.main_window.QFileDialog.getOpenFileName', return_value=(str(large_file), "All Files (*.*)")):
    ...
    assert "This is a test sentence." in main_app_window.document_display_area.toPlainText()
...
def test_corrupted_file_upload(main_app_window: MainApplicationWindow, qtbot, mock_backend_services):
    ...
    with patch('src.gui.main_window.QFileDialog.getOpenFileName', return_value=("/fake/path/corrupted.pdf", "All Files (*.*)")), \
    ...
    assert "Could not display preview" in main_app_window.document_display_area.toPlainText()
