...
def mock_all_backend_services():
    ...
    with patch("src.gui.main_window.AILoaderWorker") as mock_ai_loader, patch(
        "src.gui.main_window.DashboardWorker"
    ) as mock_dash_worker, patch(
        "src.gui.main_window.AnalysisStarterWorker"
    ) as mock_starter, patch(
        "src.gui.main_window.requests.post"
    ) as mock_post, patch(
        "src.gui.main_window.QFileDialog.getOpenFileName"
    ) as mock_open_file, patch(
        "src.gui.main_window.ChatDialog"
    ) as mock_chat_dialog:
