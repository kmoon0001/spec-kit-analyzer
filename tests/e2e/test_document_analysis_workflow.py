...
class TestDocumentAnalysisWorkflow:
    ...
    async def test_complete_analysis_workflow(
        ...
        doc_response = e2e_helper.client.get(
        ...
        rubrics_response = e2e_helper.client.get(
            "/rubrics",
            headers=e2e_helper.headers
        )
        ...
        results = e2e_helper.wait_for_analysis(
            task_id, 
            timeout=e2e_test_config["performance_thresholds"]["document_analysis"]
        )
        ...
        pdf_response = e2e_helper.client.post(
            f"/export-pdf/{task_id}",
            headers=e2e_helper.headers
        )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_invalid_document(
        self,
        e2e_helper: E2ETestHelper,
        temp_upload_dir: Path
    ):
    ...
    @pytest.mark.asyncio
    async def test_analysis_progress_monitoring(
        self,
        e2e_helper: E2ETestHelper,
        sample_document_file: Path
    ):
