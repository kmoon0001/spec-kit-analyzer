...
patchers = [
    patch('src.core.llm_service.AutoModelForCausalLM.from_pretrained', return_value=MagicMock()),
    patch('src.core.fact_checker_service.pipeline', return_value=MagicMock()),
    patch('src.core.ner.pipeline', return_value=MagicMock()),
    patch('src.core.hybrid_retriever.crud.get_rubrics', return_value=[], create=True), # Prevent DB calls and create the attribute
...
def test_analyze_document_api_route(client: TestClient, mocker):
    ...
    mock_run_analysis = mocker.patch(
        'src.api.routers.analysis.run_analysis_and_save',
        return_value=None
    )
