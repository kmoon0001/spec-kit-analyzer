...
from src.core.query_expander import ExpansionResult, MedicalVocabulary, QueryExpander, SemanticExpander
...
class TestSemanticExpander:
    ...
    def test_expand_semantically_without_model(self):
        ...
        result = expander.expand_semantically(
            "physical therapy", ["rehabilitation", "physiotherapy", "treatment"], max_expansions=3
        )
    ...
    def test_expand_semantically_with_mock_model(self):
        ...
        result = expander.expand_semantically(
            "PT", ["physical therapy", "rehabilitation", "treatment"], max_expansions=2, similarity_threshold=0.1
        )
...
class TestQueryExpansionIntegration:
    ...
    def test_realistic_medical_query_expansion(self):
        ...
        for case in test_cases:
            result = expander.expand_query(
                query=case["query"], discipline=case["discipline"], document_type=case["doc_type"]
            )
            ...
            found_expected = sum(1 for term in case["expected_expansions"] if term.lower() in expanded_query_lower)
            ...
            assert found_expected > 0 or len(result.expanded_terms) > 0, (
                f"No expected terms or expansions found for '{case['query']}'. Got: {result.get_expanded_query()}"
            )
    ...
    def test_expansion_with_medical_entities(self):
        ...
        result = expander.expand_query(query="therapy progress", discipline="pt", context_entities=medical_entities)
        ...
        for _term, confidence in result.confidence_scores.items():
