...
async def test_get_user(db_session: AsyncSession):
    user = await crud.create_user(
        db_session, schemas.UserCreate(username="testuser_crud", password="password"), "hashed_password"
    )
...
async def test_get_user_by_username(db_session: AsyncSession):
    await crud.create_user(
        db_session, schemas.UserCreate(username="testuser2_crud", password="password"), "hashed_password"
    )
...
async def test_change_user_password(db_session: AsyncSession):
    user = await crud.create_user(
        db_session, schemas.UserCreate(username="testuser3_crud", password="password"), "hashed_password"
    )
