...
class TestSystemIntegrationService:
    ...
    def test_health_status_calculation(self):
        ...
        excellent_status = service._calculate_health_status(MemoryPressureLevel.LOW, 0.95, 0.1, 0.9)
        assert excellent_status == SystemHealthStatus.EXCELLENT

        # Test critical health
        critical_status = service._calculate_health_status(MemoryPressureLevel.CRITICAL, 0.2, 0.9, 0.1)
        assert critical_status == SystemHealthStatus.CRITICAL

        # Test good health
        good_status = service._calculate_health_status(MemoryPressureLevel.MODERATE, 0.8, 0.3, 0.8)
...
class TestIntegration:
    ...
    def test_end_to_end_system_integration(self, mock_memory):
        ...
        assert initial_metrics.memory_pressure in [
    ...
    def test_auto_optimization_trigger(self, mock_memory):
        ...
        config = IntegrationConfiguration(enable_auto_optimization=True, memory_threshold_percent=90.0)
        service = SystemIntegrationService(config)
        ...
        needs_optimization = (
            ...
            or metrics.cache_hit_rate < config.cache_optimization_threshold
            or metrics.optimization_score < config.performance_threshold
        )
