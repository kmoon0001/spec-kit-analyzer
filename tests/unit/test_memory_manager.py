...
class TestMemoryMonitor:
    ...
    def test_pressure_level_detection(self, mock_swap, mock_memory):
        ...
        for memory_percent, expected_level in test_cases:
            mock_memory.return_value = Mock(
                total=16 * 1024**3, available=8 * 1024**3, used=8 * 1024**3, percent=memory_percent
            )
    ...
    def test_monitoring_lifecycle(self, mock_swap, mock_memory):
        ...
        mock_memory.return_value = Mock(total=16 * 1024**3, available=8 * 1024**3, used=8 * 1024**3, percent=50.0)
        mock_swap.return_value = Mock(used=0, percent=0.0)
...
class TestMemoryManager:
    ...
    def test_memory_status_reporting(self, mock_memory):
        ...
        mock_memory.return_value = Mock(total=16 * 1024**3, available=8 * 1024**3, percent=50.0)
    ...
    def test_pressure_based_optimization(self, mock_memory):
        ...
        high_pressure_metrics = Mock(pressure_level=MemoryPressureLevel.HIGH, memory_percent=85.0)

        with patch.object(manager.monitor, "get_current_metrics", return_value=high_pressure_metrics):
            with patch.object(manager.optimizer, "optimize_memory", return_value={"success": True}) as mock_optimize:
...
class TestIntegration:
    ...
    def test_end_to_end_memory_management(self, mock_swap, mock_memory):
        ...
        for i, resource in enumerate(resources):
            manager.register_resource(f"component_{i}", f"resource_{i}", resource, 100 * 1024 * 1024)  # 100MB each
        ...
        # Test optimization
        with patch.object(
            manager.optimizer, "optimize_memory", return_value={"success": True, "bytes_freed": 200 * 1024 * 1024}
        ):
