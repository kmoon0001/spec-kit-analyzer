...
from src.gui.core import ResourceMonitor, ResourceManager, BaseWorker, WorkerSignals, JobPriority
...
class TestTimeoutScenarios:
    ...
    def test_worker_timeout(self, qapp):
        ...
        assert error_received[0][0] is TimeoutError
    ...
    def test_multiple_timeout_workers(self, qapp):
        ...
        # All should timeout
        assert len(errors) == 5
        assert all(e[0] is TimeoutError for e in errors)
...
class TestResourceConstraints:
    ...
    def test_resource_denial(self, qapp):
        ...
        manager = ResourceManager(thread_pool=QThreadPool(), resource_monitor=monitor)
...
class TestConcurrentOperations:
    ...
    def test_rapid_job_submission(self, qapp):
        ...
        manager = ResourceManager(thread_pool=QThreadPool(), resource_monitor=ResourceMonitor(), max_queue_size=50)
        ...
        for _i in range(30):
            worker = FastWorker()
            manager.submit_job(worker, priority=JobPriority.NORMAL, on_complete=lambda r: completed.append(r))
...
class TestCancellation:
    ...
    def test_cancel_queued_job(self, qapp):
        ...
        manager = ResourceManager(thread_pool=QThreadPool(), resource_monitor=ResourceMonitor())
    ...
    def test_cancel_active_job(self, qapp):
        ...
        job_id = manager.submit_job(
            worker, priority=JobPriority.NORMAL, on_complete=lambda r: result_received.append(r)
        )
...
def test_no_crashes_under_stress(qapp):
    ...
    manager = ResourceManager(thread_pool=QThreadPool(), resource_monitor=ResourceMonitor(), max_queue_size=100)
