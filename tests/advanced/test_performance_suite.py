...
class PerformanceTestSuite:
    ...
    async def run_comprehensive_performance_tests(self) -> dict[str, Any]:
        ...
        results = {
            "test_summary": {"total_tests": 0, "passed_tests": 0, "failed_tests": 0, "total_duration": 0.0},
            "benchmarks": [],
            "system_metrics": {},
            "recommendations": [],
        }
            ...
            for benchmark in benchmarks:
                if benchmark.success_rate < 0.9:
                    recommendations.append(
                        f"Improve reliability for {benchmark.test_name} (success rate: {benchmark.success_rate:.1%})"
                    )

                if benchmark.operations_per_second < 100:
                    recommendations.append(
                        f"Optimize {benchmark.test_name} for better throughput (current: {benchmark.operations_per_second:.1f} ops/s)"
                    )

                if benchmark.duration_seconds > 5.0:
                    recommendations.append(
                        f"Reduce execution time for {benchmark.test_name} (current: {benchmark.duration_seconds:.2f}s)"
                    )
