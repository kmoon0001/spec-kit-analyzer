...
def mock_all_backend_services():
    ...
    with patch('src.gui.main_window.AILoaderWorker') as mock_ai_loader, \
...
def test_rapid_tab_switching(main_app_window: MainApplicationWindow, qtbot):
    ...
    for i in range(100):
        qtbot.waitUntil(lambda: main_app_window.tabs.setCurrentIndex(1)) # Switch to Dashboard
        qtbot.wait(10) # Small wait to allow UI to update
        qtbot.waitUntil(lambda: main_app_window.tabs.setCurrentIndex(0)) # Switch to Analysis
        qtbot.wait(10)
...
def test_repeated_analysis_start(main_app_window: MainApplicationWindow, qtbot):
    ...
    for i in range(50):
        ...
        with patch('src.gui.main_window.AnalysisStarterWorker') as mock_starter:
