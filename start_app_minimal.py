    ...
    try:
        ...
        class MinimalNERWindow(QMainWindow):
            def __init__(self):
                ...
                title.setStyleSheet("font-size: 24px; font-weight: bold; color: #007acc; margin: 10px;")
                header_layout.addWidget(title)
                
                subtitle = QLabel("Named Entity Recognition (NER) Demo - spaCy-Free Implementation")
                ...
                input_label.setStyleSheet("font-size: 16px; font-weight: bold; color: #333;")
                layout.addWidget(input_label)
                
                self.input_text = QTextEdit()
                self.input_text.setPlaceholderText("Enter clinical documentation text here...\n\nExamples:\n• Signature: Dr. Jane Smith, PT\n• Therapist: Michael Brown, COTA\n• Signed by: Dr. Emily White, OTR")
...
Patient John Doe reported significant improvement in mobility."""
            ...
            def init_ner(self):
                ...
                try:
                    ...
                    self.results_text.append("✅ Regex-based clinician name extraction: Ready")
                    self.results_text.append("✅ Presidio PHI detection: Ready")
                    self.results_text.append("✅ Medical entity categorization: Ready")
                    self.results_text.append("✅ spaCy dependency: Removed (no conflicts!)")
                    self.results_text.append("")
                    self.statusBar().showMessage("NER system ready - Enter text and click Extract")
                except Exception as e:
                ...
                try:
                    ...
                    for i, line in enumerate(lines, 1):
                        ...
                        if clinicians:
                            ...
                            self.results_text.append(f"  ℹ️ No clinicians detected")
                        self.results_text.append("")
                    ...
                    self.results_text.append("=" * 50)
                    self.results_text.append(f"Total clinicians found: {len(unique_clinicians)}")
                    if unique_clinicians:
                        self.results_text.append(f"Unique clinicians: {unique_clinicians}")
                        self.results_text.append("✅ NER extraction completed successfully!")
                        self.statusBar().showMessage(f"Success: Found {len(unique_clinicians)} clinician(s)")
                    else:
                        self.results_text.append("No clinician names detected in the text.")
                        self.results_text.append("This may be correct if the text doesn't contain clinical signatures.")
