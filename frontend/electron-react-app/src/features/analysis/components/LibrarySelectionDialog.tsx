...
const MOCK_RUBRICS: RubricItem[] = [
  ...
  },
  {
    ...
    description:
      "Customized guidelines for therapy cap exceptions and documentation",
    discipline: "all",
    category: "custom",
  ...
  },
  {
    ...
    description:
      "Organization-specific standards for skilled therapy documentation",
    discipline: "all",
    category: "custom",
...
export function LibrarySelectionDialog({
  ...
  const filteredRubrics = MOCK_RUBRICS.filter((rubric) => {
    const matchesCategory =
      categoryFilter === "all" || rubric.category === categoryFilter;
    const matchesDiscipline =
      disciplineFilter === "all" ||
      rubric.discipline === disciplineFilter ||
      rubric.discipline === "all";
    const matchesSearch =
      searchTerm === "" ||
  ...
  return (
    <div className={styles.dialogOverlay}>
      <Card
        ...
        subtitle="Select from pre-loaded compliance rubrics"
        className={className}
      >
          ...
          <div className={styles.rubricList}>
            <div className={styles.listHeader}>
              <span className={styles.headerText}>
                ...
                {filteredRubrics.length !== 1 ? "s" : ""} found
              </span>
            </div>
                ...
                >
                  <div className={styles.rubricHeader}>
                    <div className={styles.rubricTitle}>
                      <span className={styles.categoryIcon}>
                        {getCategoryIcon(rubric.category)}
                      </span>
                      <span className={styles.disciplineIcon}>
                        {getDisciplineIcon(rubric.discipline)}
                      </span>
                    ...
                    <div className={styles.rubricMeta}>
                      <span className={styles.lastModified}>
                        Modified:{" "}
                        {new Date(rubric.lastModified).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className={styles.rubricTags}>
                    <span className={styles.tag}>
                      {rubric.category.toUpperCase()}
                    </span>
                    <span className={styles.tag}>
                      {rubric.discipline.toUpperCase()}
                    </span>
                  </div>
                </div>
              ))}
