...
const HELP_CONTENT: Record<string, HelpContent> = {
    upload_button: {
        ...
        content: "Upload PDF, DOCX, or TXT files for compliance analysis. Scanned documents will be processed with OCR. Supported formats include therapy notes, evaluations, progress reports, and discharge summaries.",
        category: 'general'
    },
    rubric_selector: {
        title: "Compliance Rubric",
        content: "Select the appropriate compliance rubric for your discipline (PT, OT, SLP) to ensure accurate analysis. The system will auto-detect the most suitable rubric based on document content.",
        category: 'compliance'
    },
    analyze_button: {
        title: "Run Analysis",
        content: "Start AI-powered compliance analysis. This may take 30-60 seconds depending on document size. The system uses multiple AI models for comprehensive analysis including NER, fact-checking, and compliance scoring.",
        category: 'technical'
    },
    compliance_score: {
        title: "Compliance Score",
        content: "Overall compliance score from 0-100%. Scores above 85% indicate good compliance. Scores 70-85% suggest minor improvements needed. Scores below 70% require significant documentation improvements.",
        category: 'compliance'
    },
    findings_table: {
        title: "Compliance Findings",
        content: "Detailed list of compliance issues found in your document, with risk levels and recommendations. Each finding includes specific guidance on how to improve documentation.",
        category: 'compliance'
    },
    performance_settings: {
        title: "Performance Settings",
        content: "Adjust system performance based on your hardware. Conservative mode for 6-8GB RAM, Aggressive for 12GB+. Higher performance settings enable faster analysis but require more system resources.",
        category: 'technical'
    },
    dashboard: {
        title: "Analytics Dashboard",
        content: "View historical compliance trends, performance metrics, and insights from your analysis history. Track improvement over time and identify patterns in documentation quality.",
        category: 'general'
    },
    chat_assistant: {
        title: "AI Chat Assistant",
        content: "Get instant help with compliance questions, documentation best practices, and clinical guidelines. The assistant can explain findings, suggest improvements, and provide educational resources.",
        category: 'clinical'
    }
};
...
export function HelpSystem({ className }: HelpSystemProps) {
    ...
    return (
        <div className={styles.helpSystem}>
            <Card title="📚 Help & Resources" subtitle="Comprehensive guidance for therapy compliance documentation" className={className}>
                <div className={styles.helpContainer}>
                        ...
                        {selectedCategory === 'clinical' ? (
                            <div className={styles.clinicalGuides}>
                                <div className={styles.guideSelector}>
                                    <label className={styles.guideLabel}>Select Discipline:</label>
                                    <select
                                        value={selectedGuide}
                                ...
                                <div className={styles.guideContent}>
                                    <h3 className={styles.guideTitle}>{CLINICAL_GUIDES[selectedGuide as keyof typeof CLINICAL_GUIDES].title}</h3>
                                    <div
                                        className={styles.guideText}
                                        dangerouslySetInnerHTML={{
                                            __html: CLINICAL_GUIDES[selectedGuide as keyof typeof CLINICAL_GUIDES].content
                                        }}
                                    />
                                </div>
                            </div>
                            ...
                            <div className={styles.helpContent}>
                                <h3 className={styles.contentTitle}>
                                    {categories.find(c => c.key === selectedCategory)?.label} Resources
                                </h3>
                                    ...
                                    {filteredContent.map(([key, content]) => (
                                        <div key={key} className={styles.helpItem}>
                                            <div className={styles.helpItemHeader}>
                                                <h4 className={styles.helpItemTitle}>{content.title}</h4>
                                                <StatusChip
                                                    label={content.category.charAt(0).toUpperCase() + content.category.slice(1)}
                                                    status="ready"
                                                />
                                            </div>
                                            <p className={styles.helpItemContent}>{content.content}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className={styles.quickHelp}>
                        ...
                        <div className={styles.quickHelpButtons}>
                            {Object.entries(HELP_CONTENT).slice(0, 4).map(([key, content]) => (
                                <button
                                    key={key}
                ...
                {showTooltip && (
                    ...
                    >
                        <div className={styles.tooltipContent}>
                            <h4 className={styles.tooltipTitle}>{HELP_CONTENT[showTooltip]?.title}</h4>
                            <p className={styles.tooltipText}>{HELP_CONTENT[showTooltip]?.content}</p>
                        </div>
                    </div>
                )}
            </Card>
