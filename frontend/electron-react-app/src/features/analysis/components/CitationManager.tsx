...
const MOCK_CITATIONS: Citation[] = [
    {
        ...
        type: 'quote',
        content: 'Physical therapy services must be reasonable and necessary for the treatment of the individual\'s illness or injury.',
    ...
    },
    {
        id: 'apta_guideline_1',
        type: 'guideline',
        content: 'Physical therapists shall provide services only when they can reasonably expect to achieve beneficial outcomes.',
    ...
    },
    {
        ...
        type: 'reference',
        content: 'Documentation must be legible, complete, and clearly establish medical necessity for each service provided.',
    ...
    },
    {
        id: 'aota_standard_1',
        type: 'standard',
        content: 'Occupational therapy practitioners shall provide services in a manner that is consistent with accepted professional standards.',
    ...
    },
    {
        id: 'asha_guideline_1',
        type: 'guideline',
        content: 'Speech-language pathologists shall provide services that are evidence-based and culturally responsive.',
    ...
    },
    {
        ...
        type: 'quote',
        content: 'The therapy cap exceptions process requires documentation of medical necessity and functional improvement.',
...
export function CitationManager({ onSelect, onInsert, onClose, className }: CitationManagerProps) {
    ...
    const filteredCitations = MOCK_CITATIONS.filter(citation => {
        ...
        const matchesDiscipline = disciplineFilter === 'all' || citation.discipline === disciplineFilter || citation.discipline === 'all';
    ...
    return (
        <Card title="📚 Citation Manager" subtitle="Manage quotes, references, and guidelines" className={className}>
                            ...
                            >
                                <div className={styles.citationHeader}>
                                    <div className={styles.citationTitle}>
                                        <span className={styles.typeIcon}>{getTypeIcon(citation.type)}</span>
                                        <span className={styles.categoryIcon}>{getCategoryIcon(citation.category)}</span>
                                        <span className={styles.disciplineIcon}>{getDisciplineIcon(citation.discipline)}</span>
                                        <span className={styles.citationType}>{citation.type.toUpperCase()}</span>
                                    </div>
                                    <div className={styles.citationMeta}>
                                        <span className={styles.lastUsed}>
                                            {citation.lastUsed ? `Used: ${new Date(citation.lastUsed).toLocaleDateString()}` : 'Never used'}
                                        </span>
                                    </div>
                                </div>
                    ...
                    <div className={styles.selectedInfo}>
                        {selectedCitation ? (
                            <div className={styles.selectedCitation}>
                                ...
                                <span className={styles.selectedName}>{selectedCitation.source}</span>
                            </div>
                        ) : (
