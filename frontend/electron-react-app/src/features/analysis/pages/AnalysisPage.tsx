...
const STRICTNESS_LEVELS = [
  ...
  },
  {
    ...
    description:
      "Adds longitudinal outcomes, utilization, and PHI scrub accuracy.",
  },
];
...
const MODEL_STATUS = [
  ...
  { name: "TinyLlama Clinical AI", status: "ready" as const },
...
export default function AnalysisPage() {
  ...
  const analysisError =
    controller.error instanceof Error ? controller.error : null;
  const progressPercent = Math.max(
    0,
    Math.min(100, Math.round(controller.progress ?? 0)),
  );
  const telemetryCpu =
    telemetry.metrics.cpuPercent !== null
      ? `${telemetry.metrics.cpuPercent.toFixed(1)}%`
      : "--";
  const telemetryMemory =
    telemetry.metrics.memoryMb !== null
      ? `${Math.round(telemetry.metrics.memoryMb)} MB`
      : "--";
  const telemetryLoop =
    telemetry.metrics.eventLoopP99 !== null
      ? `${telemetry.metrics.eventLoopP99.toFixed(1)} ms`
      : "--";
  ...
  const telemetryUpdatedLabel = telemetry.lastUpdatedAt
    ? new Date(telemetry.lastUpdatedAt).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      })
  ...
  const statusLabel = controller.status
    ? controller.status.replace(/_/g, " ")
    ...
    <div className={styles.wrapper}>
      <section className={styles.overviewStrip}>
        <div>
          ...
          <p>
            Upload therapy documentation, select evaluation targets, and
            orchestrate AI-powered scoring that mirrors the PySide workflows
            while staying responsive on desktop.
          </p>
          <div className={styles.runtimePillRow}>
            <StatusChip
              label={
                controller.isDesktop ? "Desktop Runtime" : "Browser Runtime"
              }
              status={controller.isDesktop ? "ready" : "warming"}
            />
            <StatusChip
              label={controller.isPolling ? "Task Active" : "Idle"}
              status={controller.isPolling ? "ready" : "offline"}
            />
          </div>
        </div>
        <div className={styles.overviewStats}>
          <Card
            ...
            subtitle={controller.selectedFile?.name ?? "Awaiting document"}
          >
            <div className={styles.metricRow}>
              ...
              <div>
                ...
                <span className={styles.metricValue}>
                  {controller.taskId || controller.isPolling
                    ? `${progressPercent}%`
                    : "0%"}
                </span>
              </div>
                ...
                <ul className={styles.telemetryWorkerList}>
                  {telemetry.snapshot.workers.slice(0, 3).map((worker) => (
                    <li key={worker.jobId}>
                      ...
                      <span>
                        {Math.max(0, Math.round(worker.runtimeMs / 1000))}s
                      </span>
                    </li>
                  ))}
                </ul>
      ...
      <section className={styles.columns}>
        <div className={styles.columnLeft}>
          ...
          >
              ...
              {controller.selectedFile ? (
                <>
                  ...
                  <p className={styles.dropzoneHint}>
                    {(controller.selectedFile.size / 1024 / 1024).toFixed(2)} MB
                    - Stored locally until you start analysis
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className={styles.dropzoneHint}>
                    HIPAA-safe: heavy processing offloads to secure workers.
                  </p>
                </>
              )}
            </div>
            ...
            {analysisError && (
              <div className={styles.errorBanner}>{analysisError.message}</div>
            )}
            ...
            <div className={styles.dropzoneActions}>
              ...
              <Button
                ...
                disabled={!controller.isPolling || !controller.isDesktop}
              >
                Cancel
              </Button>
              <Button
            ...
            <p className={styles.intakeHint}>
              Background workers stream progress without blocking the UI. Cancel
              gracefully when inference takes too long.
            </p>
          </Card>

          <Card
            title="Discipline & Rubrics"
            subtitle="Auto-detected based on document content"
          >
            <div className={styles.selectRow}>
              ...
              <select
                ...
                onChange={(event) =>
                  controller.setDiscipline(event.target.value)
                }
              >
              ...
              {controller.autoDetectedDiscipline && (
                <span className={styles.autoDetected}>
                  🤖 Auto-detected:{" "}
                  {controller.autoDetectedDiscipline.toUpperCase()}
                </span>
              )}
            </div>
            ...
            <div className={styles.selectRow}>
              ...
              <select
                ...
                onChange={(event) =>
                  controller.setSelectedRubric(event.target.value)
                }
              >
                <option value="default">
                  📋 Medicare Benefits Policy Manual - Chapter 15
                </option>
                <option value="medicare_part_b">
                  📋 Medicare Part B Outpatient Therapy Guidelines
                </option>
                <option value="cms_1500">
                  📋 CMS-1500 Documentation Requirements
                </option>
                <option value="therapy_cap">
                  📋 Medicare Therapy Cap & Exception Guidelines
                </option>
                <option value="skilled_therapy">
                  📋 Skilled Therapy Documentation Standards
                </option>
                <option value="apta_pt">
                  🏃 Physical Therapy - APTA Guidelines
                </option>
                <option value="aota_ot">
                  🖐️ Occupational Therapy - AOTA Standards
                </option>
                <option value="asha_slp">
                  🗣️ Speech-Language Pathology - ASHA Guidelines
                </option>
              </select>
            ...
            <div className={styles.rubricDescription}>
              <p className={styles.description}>
                {controller.selectedRubric === "default" &&
                  "Comprehensive Medicare coverage policies for medical services including therapy requirements."}
                {controller.selectedRubric === "medicare_part_b" &&
                  "Specific guidelines for outpatient therapy services under Medicare Part B."}
                {controller.selectedRubric === "cms_1500" &&
                  "Documentation standards required for CMS-1500 claim forms."}
                {controller.selectedRubric === "therapy_cap" &&
                  "Guidelines for therapy cap exceptions and documentation requirements."}
                {controller.selectedRubric === "skilled_therapy" &&
                  "Standards for documenting skilled therapy services and medical necessity."}
                {controller.selectedRubric === "apta_pt" &&
                  "American Physical Therapy Association clinical practice guidelines."}
                {controller.selectedRubric === "aota_ot" &&
                  "American Occupational Therapy Association practice standards."}
                {controller.selectedRubric === "asha_slp" &&
                  "American Speech-Language-Hearing Association clinical guidelines."}
              </p>
              {controller.rubricSuggestions &&
                controller.rubricSuggestions.length > 1 && (
                  <div className={styles.rubricSuggestions}>
                    <p className={styles.suggestionLabel}>
                      Other suggested rubrics:
                    </p>
                        ...
                        .map((suggestion, index) => (
                          <li key={index} className={styles.suggestionItem}>
                            <button
                              ...
                              onClick={() =>
                                controller.setSelectedRubric(
                                  suggestion.rubric_id,
                                )
                              }
                            >
                              {suggestion.rubric_id
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className={styles.strictnessBody}>
                        {level.description}
                      </p>
                    </div>
                  </button>
          ...
          <Card
            ...
            subtitle="Matches ReportGenerator options"
          >
          ...
          <Card
            title="Licenses & Safeguards"
            subtitle="Parity with PySide guardrails"
          >
            <p className={styles.description}>
              Electron reuses backend license checks and audit logging.
              Background workers surface crash signals so desktop ops stay
              stable.
            </p>
        ...
        <div className={styles.columnRight}>
          <Card
            ...
            subtitle="Parity with StatusComponent"
          >
          ...
          <Card
            ...
            subtitle="Structured output from AnalysisService"
          >
            {!controller.taskId && (
              <p className={styles.description}>
                Run an analysis to populate clinical findings.
              </p>
            )}
            {controller.taskId &&
              !controller.analysisComplete &&
              !controller.analysisFailed && (
                <p className={styles.description}>
                  Analysis running. Findings stream in once complete.
                </p>
              )}
            {controller.analysisFailed && (
              <p className={styles.errorText}>
                Analysis failed. Check logs or retry when resources recover.
              </p>
            )}
            {controller.summary && controller.analysisComplete && (
              <ul className={styles.findingsList}>
                {findings.slice(0, 5).map((finding, index) => {
                  ...
                  const severityValue = (
                    finding as { severity?: string | number }
                  )?.severity;
                  const severityLabel =
                    severityValue !== undefined
                      ...
                      : "Info";
                  const statusVariant =
                    severityLabel.toLowerCase() === "high"
                      ? "warning"
                      : "warming";
                  ...
                  return (
                    <li key={key}>
                      <div className={styles.findingHeader}>
                        <span className={styles.findingId}>
                          {(finding as { id?: string })?.id ?? `F-${index + 1}`}
                        </span>
                        <StatusChip
                          label={severityLabel}
                      ...
                      <p>
                        {(finding as { description?: string; summary?: string })
                          ?.description ??
                          (
                            finding as {
                              description?: string;
