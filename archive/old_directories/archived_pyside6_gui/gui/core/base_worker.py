...
class BaseWorker(QRunnable, metaclass=ABCQRunnable):
    ...
    def __init__(
        self, timeout_seconds: float = 300.0, resource_monitor: ResourceMonitor | None = None, job_type: str = "general"
    ):
        """
    ...
    def run(self):
        ...
        try:
            ...
            if not can_start:
                ...
                self.signals.error.emit((ResourceError, ResourceError(reason), reason))
                return
            ...
            self.signals.error.emit((type(e), e, f"Operation timed out after {self.timeout_seconds} seconds"))

        except Exception as e:
            # Any other error
            logger.error(f"{self.__class__.__name__} failed: {e}")
            exc_traceback = traceback.format_exc()
            self.signals.error.emit((type(e), e, exc_traceback))
