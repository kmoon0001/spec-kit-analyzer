...
#### Upload Document
```bash
curl -X POST "http://localhost:8001/api/analysis/analyze" \
...
#### Check Status
```bash
curl -X GET "http://localhost:8001/api/analysis/status/abc123" \
  -H "Authorization: Bearer <token>"
```

#### Export PDF
```bash
curl -X POST "http://localhost:8001/api/analysis/export-pdf/abc123" \
  -H "Authorization: Bearer <token>"
```
